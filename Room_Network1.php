...
// --- ADMIN TOGGLE ---
$admin = "No"; // Set to "No" to hide all editing and management features
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $admin === "Yes") {
    ...
    if ($action === 'add' || $action === 'edit') {
        ...
        if ($action === 'add') {
            $sql = "INSERT INTO `igpess_network` (`floor`, `room_no`, `description`, `icon`, `networking`, `interactive_board`, `wifi_router`, `cctv`, `ups`, `audio_video`, `remarks`, `room_image`, `width`, `length`, `latitude`, `longitude`, `room_members`, `in_charge_id`)
                    VALUES ('$floor', '$room_no', '$description', '$icon', $networking, $int_board, $wifi, $cctv, $ups, $audio_video, $remarks, $room_image_val, $width, $length, $latitude, $longitude, $room_members, $in_charge_id)";
        } else {
            $id = (int)$_POST['id'];
...
if ($org_result && $org_result->num_rows > 0) {
    while($row = $org_result->fetch_assoc()) {
        ...
        // DYNAMIC NAME SPLITTER: Breaks names separated by <br>, <br/>, or \n
...
if ($result && $result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        // Add Split Staff Member Names & Roles to Search Index!
...
<head>
    ...
    <style>
        ...
        .accordion-button:focus {
            box-shadow: none;
            border-color: rgba(30, 60, 114, 0.125);
        }
        .accordion-button {
            font-weight: 600;
        ...
        .interactive-room:nth-child(even) .map-room { animation-delay: 1.5s; }
        .interactive-room:nth-child(3n) .map-room { animation-delay: 3s; }
...
<body>
    ...
    <div class="container-fluid pb-5 mt-4 px-4">
        ...
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-center mb-4 bg-white p-3 rounded-4 shadow-sm border no-print gap-3">
            ...
            <div class="input-group w-auto flex-grow-1" style="max-width: 500px;">
                ...
                <input type="text" id="globalSearch" class="form-control bg-light border-start-0 rounded-end-pill shadow-none py-2" placeholder="Search for rooms, staff names, hardware, Wi-Fi, A/V...">
            </div>
        ...
        <div class="tab-content">
            <!-- 3D Horizontal Map Tab - Always First and Active -->
                ...
                <div class="map-wrapper mb-5">
                    ...
                    <svg class="svg-container" viewBox="0 0 2800 1100">
                        ...
                        foreach($svgFloors as $fName => $fData):
                            ...
                            $accordionTarget = 'collapse-' . md5($fName);
                        ?>
                        <g class="isometric-layer <?= $layerClass ?>">
                            ...
                            <g class="staircase-group" transform="translate(200, 200)" onclick="scrollToAccordion('<?= $accordionTarget ?>')">
            ...
            <!-- Accordion for Floor-wise Rooms - All Closed by Default -->
            <div class="accordion no-print" id="floorAccordion">
                <?php foreach(["Ground Floor", "First Floor", "Second Floor"] as $floorName): ?>
                    <?php if(empty($buildingData[$floorName])) continue; ?>
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="heading-<?= md5($floorName) ?>">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-<?= md5($floorName) ?>" aria-expanded="false" aria-controls="collapse-<?= md5($floorName) ?>">
                                <i class="fas fa-layer-group me-2"></i><?= strtoupper($floorName) ?> (<?= count($buildingData[$floorName]) ?> Rooms)
                            </button>
                        </h2>
                        <div id="collapse-<?= md5($floorName) ?>" class="accordion-collapse collapse" data-bs-parent="#floorAccordion">
                            <div class="accordion-body">
                                <div class="row g-4" id="roomsContainer-<?= md5($floorName) ?>">
                                    <?php foreach($buildingData[$floorName] as $room): ?>
                                        <div class="col-xl-3 col-lg-4 col-md-6 col-sm-12 room-card-wrapper" data-search="<?= htmlspecialchars($room['search_index']) ?>">
                                            <div class="room-card" data-tilt data-tilt-max="10" data-tilt-speed="400" onclick="openViewModal('<?= addslashes(htmlspecialchars($room['room_no'])) ?>')" style="cursor: pointer;">
                                                ...
                                                <?php if ($admin === "Yes"): ?>
                                                <div class="edit-icon-card text-muted" onclick='event.stopPropagation(); openModal("edit", <?= json_encode($room, JSON_HEX_APOS | JSON_HEX_QUOT) ?>, "map")' title="Edit Room"><i class="fas fa-edit"></i></div>
                                                <?php endif; ?>
                                                ...
                                                <div class="room-card-inner">
                                                    ...
                                                    <div class="hardware-badges mt-2 border-top pt-2">
                                                        ...
                                                        <?php if(empty($room['networking']) && empty($room['interactive_board']) && empty($room['wifi_router']) && empty($room['cctv']) && empty($room['ups']) && empty($room['audio_video'])) echo '<span class="text-muted small">No hardware</span>'; ?>
                                                    </div>
                                                    ...
                                                    <div class="d-flex justify-content-center gap-2 align-items-center mt-2 flex-wrap">
                                                        ...
                                                        <?php if(!empty($room['latitude']) && !empty($room['longitude'])): ?>
                                                            <a href="https://www.google.com/maps?q=<?= htmlspecialchars($room['latitude']) ?>,<?= htmlspecialchars($room['longitude']) ?>" target="_blank" onclick="event.stopPropagation()" class="badge bg-dark text-white text-decoration-none p-1 shadow-sm border border-secondary" title="Open Location in Google Maps">
                                                    ...
                                                    <div class="mt-3 w-100 text-start bg-light p-2 rounded shadow-sm border card-personnel-block">
                                                        ...
                                                        <?php
                                                        // Sort to bring In-Charge to the top
                                                    ...
                                                    <?php if(!empty($room['remarks'])): ?><div class="room-remarks"><i class="fas fa-info-circle"></i> <?= htmlspecialchars($room['remarks']) ?></div><?php endif; ?>
                                                </div>
                                            </div>
                                        </div>
                                    <?php endforeach; ?>
            ...
            <div class="print-container d-none d-print-block">
                ...
                <table class="print-table bg-white">
                    <thead>
                        <tr>
                            <th width="6%">Room</th><th width="15%">Description</th><th width="8%">Network</th><th width="8%">Int. Board</th><th width="8%">Wi-Fi</th><th width="8%">CCTV</th><th width="7%">UPS</th><th width="6%">A/V</th><th width="9%">Sq.Ft</th><th width="25%">Remarks</th>
                        </tr>
                    </thead>
                    ...
                    <tbody class="floor-group">
                        ...
                        <?php foreach($rooms as $room): ?>
                            <tr>
                                ...
                                <td class="text-center-print"><?= !empty($room['sqft']) ? $room['sqft'] . ' ft²' : '-' ?></td>
                                            ...
                                            foreach($membersArr as $mid) {
                                                if(isset($staffMap[$mid])) {
                                                    ...
                                                    if($mid == $room['in_charge_id']) {
                                                        $name = "<strong>{$name} (? In-Charge)</strong>";
                                                    }
                                                    $staffNames[] = $name;
                                                }
                                            }
            ...
            <div class="modal-content">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <h6 class="text-primary border-bottom pb-2 mt-2">3. Dimensions & Location (OpenStreetMap)</h6>
                        <div class="row">
                        ...
                        <h6 class="text-primary border-bottom pb-2 mt-2">4. IT & Hardware Setup</h6>
                        <div class="row">
    ...
    <!-- Room View Modal -->
    <div class="modal fade no-print" id="mapInfoModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                ...
                <div class="modal-body text-center bg-light">
                    ...
                    <div class="mb-4">
                        <span class="badge bg-secondary px-3 py-2 fs-6 shadow-sm"><i class="fas fa-ruler-combined me-2"></i><span id="mapViewDimensions">Dimensions Not Set</span></span>
                    </div>
                    ...
                    <div id="viewMapWrapper" class="text-start d-none">
                        ...
                        <div id="viewMap" class="shadow-sm" style="height: 250px; width: 100%;"></div>
                    </div>
    ...
    <script>
        ...
        // Function to scroll to accordion and open it
        function scrollToAccordion(targetId) {
            const accordionElement = document.getElementById(targetId);
            if (accordionElement) {
                const button = document.querySelector(`button[data-bs-target="#${targetId}"]`);
        ...
        document.getElementById('globalSearch').addEventListener('input', function(e) {
            ...
            document.querySelectorAll('.room-card-wrapper').forEach(card => {
                ...
                if (searchData && checkMatch(searchData)) {
                    card.style.display = '';
                } else {
            ...
            document.querySelectorAll('.interactive-room').forEach(svgRoom => {
                ...
                if (searchData && (checkMatch(searchData) || e.target.value.trim() === '')) {
        ...
        function openViewModal(roomNo) {
            ...
            if (roomData) {
                ...
                // Render Members inside Map Modal
                let membersHtml = '';
                ...
                // Handle map
                if (roomData.latitude && roomData.longitude && roomData.latitude != 'NULL' && roomData.longitude != 'NULL') {
                    ...
                    document.getElementById('mapViewGoogleLink').href = `https://www.google.com/maps?q=${lat},${lng}`;
                ...
                if (roomData.remarks && roomData.remarks !== 'NULL') {
                ...
                // Show modal
                const modalElement = document.getElementById('mapInfoModal');
        ...
        const staffSearchInput = document.getElementById('staffSearchModal');
        if (staffSearchInput) {
        ...
        // Auto-check the member box if they are selected as In-Charge
        ...
        if (widthInput) widthInput.addEventListener('input', calculateSqFt);
        if (lengthInput) lengthInput.addEventListener('input', calculateSqFt);
        ...
        if (modalLat) modalLat.addEventListener('change', updateAdminPin);
        if (modalLng) modalLng.addEventListener('change', updateAdminPin);
        ...
        function openModal(action, data = null, tabId = 'manage') {
            ...
            if (action === 'edit' && data) {
                ...
                const fields = ['Id','RoomNo','Desc','Net','Board','Wifi','Cctv','Ups','Av','Remarks','OldImage','Width','Length','Lat','Lng','Sqft'];
                fields.forEach(f => {
                    const el = document.getElementById('modal'+f);
                    if (el) el.value = '';
                });
                const iconEl = document.getElementById('modalIcon');
                if (iconEl) iconEl.value = 'fa-door-open';
                const wrapper = document.getElementById('currentImageWrapper');

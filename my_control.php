<?php
// ==========================================
// WORKING SOLUTION: Uses SoundVolumeView.exe
// ==========================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['volume'])) {
        ...
        try {
                if (stristr($os, 'Darwin')) {
                        ...
                        // WINDOWS: Use SoundVolumeView.exe for taskbar control
                        $svclVolume = intval($volume * 655.35);

                        // Check if SoundVolumeView exists
                        $svclPath = __DIR__ . '/SoundVolumeView.exe';

                        if (file_exists($svclPath)) {
                            // Set master volume and unmute
                            shell_exec("\"$svclPath\" /SetVolume $svclVolume 2>&1");
                            shell_exec("\"$svclPath\" /Unmute 2>&1");
                            $message = "Windows volume set to $volume%";
                            $success = true;
                        } else {
                            // Alternative: Try using nircmd with correct syntax
                            $nircmdPath = __DIR__ . '/nircmd.exe';
                            if (file_exists($nircmdPath)) {
                                $nircmdVolume = intval($volume * 655.35);
                                shell_exec("\"$nircmdPath\" setsysvolume $nircmdVolume 2>&1");
                                $message = "Windows volume set to $volume% (using nircmd)";
                                $success = true;
                            } else {
                                $message = "Error: SoundVolumeView.exe not found. Please upload it.";
                                $success = false;
                            }
                        }
                        ...
                        // Linux
                        shell_exec("amixer -D pulse sset Master $volume% 2>&1");
                        $message = "Linux volume set to $volume%";
                        $success = true;
                }

                echo json_encode(['status' => $success ? 'success' : 'error', 'volume' => $volume, 'msg' => $message]);

        } catch (Exception $e) {
                echo json_encode(['status' => 'error', 'msg' => 'Failed to execute command: ' . $e->getMessage()]);
        }
        exit;
}
?>
...
<head>
        ...
        <title>AI Hand Gesture Volume Control - Working Version</title>
...
<div class="container app-container">
        ...
        <div class="alert alert-info alert-dismissible fade show mb-3" role="alert">
                ...
                <strong>Working Mode:</strong> Volume changes will affect your Windows taskbar slider.
...
<script>
        // ==========================================
        // MEDIAPIPE HAND TRACKING WITH VOLUME CONTROL
        // ==========================================
        ...
        function sendVolumeToPHP(volume) {
                const now = Date.now();
                if (now - lastSentTime > 200 && Math.abs(volume - lastSentVolume) >= 2) {
        ...
        function onResults(results) {
                ...
                if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
                        ...
                        statusText.innerText = "? Hand Detected - Pinch to Control Volume";
                        ...
                        // Map distance to volume (adjust these values for your hand size)
                        const minDistance = 0.04;
                        const maxDistance = 0.20;

                        let volume = ((distance - minDistance) / (maxDistance - minDistance)) * 100;
                        volume = Math.max(0, Math.min(100, volume));
                        const finalVolume = Math.round(volume);
                        ...
                        statusText.innerText = "? No Hand Detected - Show Your Hand to Camera";
                }
                canvasCtx.restore();
        }
        ...
        camera.start().then(() => {
                statusText.innerText = "? Camera Ready - Show Your Hand";
        }).catch(err => {
